...
| | |
|---|---|
| **Commands** | `/help` `/model` `/login` `/logout` `/mode` `/effort` `/config` `/plan` `/go` `/undo` `/rewind` `/copy` `/diff` `/context` `/compact` `/btw` `/theme` `/style` `/vim` `/memory` `/export` `/agents` `/watch` `/sessions` `/resume` `/rename` `/fork` `/handoff` `/clear` `/usage` `/skills` `/<skill> [task]` `/init` `/review` `/security-review` `/commit` `/pr` `/permissions` `/bug` `/doctor` `/mcp` `/tools` `/update` `/exit` |
