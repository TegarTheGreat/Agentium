...
**Commands.** `/init` writes `AGENTS.md`, `/review` reviews the current changes (or the branch's commits when nothing is uncommitted), `/security-review` looks for vulnerabilities an attacker could exploit in them, `/commit` commits them in the repository's style and `/pr` opens a pull request. Your own commands are Markdown files in `.agentium/commands/` or `.claude/commands/` (in the project, `~/.agentium/commands` or `~/.claude/commands`); `$ARGUMENTS` is replaced by what follows the command, and `$1` … `$9` by each word of it ("quoted words" count as one). They cannot replace a built-in command.
