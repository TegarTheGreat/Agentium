...
| | |
|---|---|
| **Commands** | `/help` `/model` `/login` `/logout` `/mode` `/effort` `/config` `/plan` `/go` `/undo` `/rewind` `/copy` `/diff` `/context` `/compact` `/btw` `/theme` `/style` `/vim` `/memory` `/export` `/agents` `/watch` `/sessions` `/resume` `/rename` `/fork` `/handoff` `/clear` `/usage` `/cost` `/skills` `/<skill> [task]` `/init` `/review` `/security-review` `/commit` `/pr` `/permissions` `/bug` `/release-notes` `/doctor` `/mcp` `/hooks` `/tools` `/update` `/exit` |
| **Typing** | `/` shows commands with what they do · `@` fuzzy-finds project files · big pastes become `[Pasted text #1 +40 lines]` chips · `Ctrl-J`, `Shift-Enter` or a trailing `\` for a new line · `Ctrl-R` searches earlier messages · `Ctrl-G` writes the message in `$EDITOR` · `↑`/`↓` move between lines, then through history · `Ctrl-K`/`Ctrl-U`/`Ctrl-W` cut and `Ctrl-Y` pastes back · `Ctrl-_` undoes · `Ctrl-S` puts a draft aside · `Alt-P`/`Alt-T` switch model and effort · `Ctrl-V` pastes an image from the clipboard (or drag an image file in) · `Ctrl-Z` suspends to the shell (`fg` returns) · `Ctrl-B` sends a running command to the background |
...
**Commands.** `/init` writes `AGENTS.md`, `/review` reviews the current changes (or the branch's commits when nothing is uncommitted), `/security-review` looks for vulnerabilities an attacker could exploit in them, `/commit` commits them in the repository's style and `/pr` opens a pull request. Your own commands are Markdown files in `.agentium/commands/` or `.claude/commands/` (in the project, `~/.agentium/commands` or `~/.claude/commands`); `$ARGUMENTS` is replaced by what follows the command, and `$1` … `$9` by each word of it ("quoted words" count as one). A line like `` !`git diff --stat` `` is replaced by that command's output when you use the command, for live context. Your own commands run theirs; a repository's commands ask first, and one-shot runs leave them as written. Front matter can give a `description` and an `argument-hint` for the `/` menu, and a `model` that runs that command's turn (the session's model comes back after). They cannot replace a built-in command.
