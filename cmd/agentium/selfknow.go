...
- Skills: "agentium skills add <dir | git URL | owner/repo>", or create <skills dir>/<name>/SKILL.md; /<name> runs one. Custom commands: .agentium/commands/<name>.md (or .claude/commands, in the project or %s) whose text is sent as the message, $ARGUMENTS replaced; /init writes AGENTS.md, /review reviews the diff. Hooks (config.json only): "hooks": {"post_edit": ["gofmt -w {path}"], "stop": ["…"], "pre_tool": [{"match": "bash", "command": "…"}] (call JSON on stdin; exit 2 blocks, stderr says why), "user_prompt": ["…"] (prints context; exit 2 stops the send), "session_start": ["…"]}. /commit, /pr, /fork; "agentium --worktree NAME" works in its own git worktree (branch agentium/NAME); /permissions lists approvals kept per project; /doctor checks the setup. Shortcuts: "keys": {"ctrl+x": "/diff", "f5": "run the tests"} (ctrl+letter, alt+key, f1-f12; agentium's own keys are kept). Other settings: "vim", "dirs", "status_line", "suggest", "subagent_model". Providers: "agentium login <provider>" or /login; custom ones under "providers".`,
