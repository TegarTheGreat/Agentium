...
- MCP servers: in the settings, "mcp": {"<name>": {"command": "npx", "args": ["-y", "pkg"], "env": {"TOKEN": "$TOKEN"}}} or {"<name>": {"url": "https://host/mcp"}}; remote OAuth with "agentium mcp login <name>"; loaded when a session starts; /mcp shows their status, tools and logs, /hooks the hooks, /usage (or /cost) tokens and cost, /doctor checks the whole setup.
- Keys: Esc stops a turn (Esc Esc rewinds), Ctrl-B sends the running command to the background, Ctrl-Z suspends at the prompt, Ctrl-O shows step outputs, Ctrl-T the side panel, Ctrl-G writes the message in $EDITOR, Ctrl-V pastes an image, Shift-Tab changes the approval mode; ? lists them all. New skills and commands work without a restart; /release-notes shows what changed.
- Skills: "agentium skills add <dir | git URL | owner/repo>", or create <skills dir>/<name>/SKILL.md; /<name> runs one. Custom commands: .agentium/commands/<name>.md (or .claude/commands, in the project or %s) whose text is sent as the message, $ARGUMENTS and $1…$9 replaced, a command in backticks after ! replaced by its output (a repository's commands ask first); /init writes AGENTS.md, /review reviews the diff, /security-review looks for exploitable vulnerabilities, /bug opens a prefilled issue for Agentium; command front matter may set description, argument-hint and model (that turn runs on it). Hooks (config.json only): "hooks": {"post_edit": ["gofmt -w {path}"], "stop": ["…"], "pre_tool": [{"match": "bash", "command": "…"}] (call JSON on stdin; exit 2 blocks, stderr says why), "user_prompt": ["…"] (prints context; exit 2 stops the send), "session_start": ["…"]}. /commit, /pr, /fork; "agentium --worktree NAME" works in its own git worktree (branch agentium/NAME); /permissions lists approvals kept per project; /doctor checks the setup. Specialist sub-agents: .agentium/agents/<name>.md (or .claude/agents), front matter name, description, tools (Read, Grep, Bash …), model, then instructions; /agents lists them. Shortcuts: "keys": {"ctrl+x": "/diff", "f5": "run the tests"} (ctrl+letter, alt+key, f1-f12; agentium's own keys are kept). Other settings: "vim", "reduce_motion", "dirs", "status_line", "suggest", "subagent_model", "oracle_model" (a stronger model the agent consults through its oracle tool). Providers: "agentium login <provider>" or /login; custom ones under "providers".`,
