...
- MCP servers: in the settings, "mcp": {"<name>": {"command": "npx", "args": ["-y", "pkg"], "env": {"TOKEN": "$TOKEN"}}} or {"<name>": {"url": "https://host/mcp"}}; remote OAuth with "agentium mcp login <name>"; loaded when a session starts; /mcp shows their status, tools and logs, /doctor checks the whole setup.
