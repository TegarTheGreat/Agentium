...
var bashTool = Tool{
	Def: providerDef("bash",
		"Run a shell command in the workspace; output clipped head+tail. Writes outside the workspace and outbound network are blocked unless net=true (installs, downloads, git push); servers may listen and local servers can be reached without it. background=true for servers/watchers/REPLs returns a job id: {job} reads new output (waits up to timeout s), {job,stdin} sends input (control chars ok, e.g. \\u0003), {job,kill} stops; {} lists jobs. tty=true gives the job a terminal (REPLs, prompts, ssh).",
